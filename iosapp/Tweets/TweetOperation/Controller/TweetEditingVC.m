...
#import "OSCTopicChoseViewController.h"//话题界面
...
@interface TweetEditingVC () <UIActionSheetDelegate, UIImagePickerControllerDelegate, UINavigationControllerDelegate, UITextViewDelegate, UIScrollViewDelegate ,UICollectionViewDelegate ,UICollectionViewDataSource,TZImagePickerControllerDelegate,YYTextViewDelegate,TopicChoseVCDelegate,UIGestureRecognizerDelegate>

@property (nonatomic, strong) YYTextView            *edittingArea;
@property (nonatomic, strong) UILabel               *textLengthLabel;
@property (nonatomic, strong) UIToolbar             *toolBar;
@property (nonatomic, strong) OSCForwardView        *forwardView;
...
- (instancetype)initWithImage:(UIImage *)image
{
    ...
    if (self) {
        ...
        [_imagesData addObject:UIImagePNGRepresentation(image)];
    }
                    ...
                    forwardItem:(OSCAbout* )forwardItem
{
    ...
    if (self) {
        ...
        self.forwardView = [[OSCForwardView alloc] initWithType:OSCForwardViewSource_editorUI];
        self.forwardView.forwardItem = forwardItem;
        self.forwardView.canEnterDetailPage = NO;
    }
...
{
    self = [self initWithAboutID:aboutID aboutType:aboutType forwardItem:forwardItem];
    if (self) {
        _fromID = fromID;
        _attribute = attribute;
        _isShowComment = isShow;
    }
    return self;
}
...
- (void)viewDidLoad
{
    ...
    [self initSubViews];
    [self setLayout];
    if (!_edittingArea.isFirstResponder) {
        [_edittingArea becomeFirstResponder];
    }
    
    if (!_edittingArea.text.length) {
        _edittingArea.attributedText = [Utils handle_TagString:[Config getTweetText] fontSize:16];
    }
}
...
- (void)initSubViews
{
    ...
    _edittingArea.placeholderText = @"今天你动弹了吗？";
    ...
    _edittingArea.tintColor = [UIColor colorWithHex:0x01A83A];
    _edittingArea.typingAttributes = @{NSForegroundColorAttributeName : [UIColor blackColor],
                                       NSFontAttributeName : [UIFont systemFontOfSize:16]};

    if (_attribute) {
        _edittingArea.attributedText = [Utils handle_TagString:_attribute.string fontSize:16];
        _edittingArea.selectedRange = NSMakeRange(0, 0);
    }
    ...
    ///< 文本长度label
    UILabel* textLengthLabel = [UILabel new];
    ...
    _textLengthLabel.backgroundColor = [UIColor whiteColor];
    _textLengthLabel.layer.masksToBounds = YES;
    _textLengthLabel.layer.cornerRadius = 5;
    ...
    NSString *imageName = (_aboutId > 0) ? @"toolbar-image_enable" : @"toolbar-image";//
    NSArray *iconName = @[imageName, @"toolbar-mention", @"toolbar-reference", @"toolbar-emoji"];
    ...
    if (_isShowComment) {
        ...
        _commentBackView = backView;
        [self.view addSubview:_commentBackView];
        UIButton *button = [UIButton buttonWithType:UIButtonTypeCustom];
        button.frame = CGRectMake(8, 0, 104, 32);
        [button setTitle:@"同时评论" forState:UIControlStateNormal];
        button.titleLabel.font = [UIFont systemFontOfSize:14.0];
        [button setImage:[UIImage imageNamed:@"form_checkbox_normal"] forState:UIControlStateNormal];
        [button setTitleColor:[UIColor colorWithHex:0x6a6a6a] forState:UIControlStateNormal];
        [button setImage:[UIImage imageNamed:@"form_checkbox_checked"] forState:UIControlStateSelected];
        ...
        _commentBtn = button;
        [_commentBackView addSubview:_commentBtn];
    }
}
...
-(void)setLayout{
    ...
    if (_isShowComment) {
        [_commentBackView mas_makeConstraints:^(MASConstraintMaker *make) {
            ...
            make.bottom.equalTo(_toolBar.mas_top).offset(-1);
    ...
    if (self.forwardView) {
        [self.forwardView mas_makeConstraints:^(MASConstraintMaker *make) {
            ...
            make.top.equalTo(self.view).offset((359*190)/(CGRectGetWidth(self.view.frame)-16));
            make.height.equalTo(@(self.forwardView.forwardItem.viewHeight));
        }];
    }
    
    [_textLengthLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.width.equalTo(@60);
        make.height.equalTo(@30);
        make.right.equalTo(self.view).offset(-6);
        make.bottom.equalTo(_toolBar.mas_top).offset(-6);
    }];
}
...
-(void)showLargeImageWithIndexPath:(NSIndexPath *)indexPath{
    ...
    if (imageIndex < _images.count - 1) {
        ...
        photoGroup.isUsingThumbViewDeedBG = YES;
...
{
    if (_edittingArea.text.length > 0 && !_isShowComment) {
        if (_teamID) {
        ...
        for (ImageCollectionViewCell* imageCell in _collectionView.visibleCells) {
            imageCell.image = nil;
        }
        _collectionView = nil;
...
- (void)insertString:(NSString *)string andSelect:(BOOL)shouldSelect
{
    ...
    NSMutableAttributedString* mAttStr = [_edittingArea attributedText].mutableCopy;

    NSUInteger cursorLocation = _edittingArea.selectedRange.location;
    NSAttributedString* resultStr = [Utils handle_TagString:string fontSize:16];

    [mAttStr insertAttributedString:resultStr atIndex:cursorLocation];
    [_edittingArea setAttributedText:mAttStr.copy];
    
    [_edittingArea setSelectedRange:NSMakeRange(cursorLocation + resultStr.length, 0)];
...
- (void)uploadTweetImages {
    ...
    NSString *urlStr = [NSString stringWithFormat:@"%@%@", OSCAPI_V2_PREFIX, OSCAPI_RESOURCE_IMAGE];
...
//    9752556
	...
	if([txtContent isEqualToString:@"#请输入软件名#"]) {
		...
		NSString *urlStr = [NSString stringWithFormat:@"%@%@", OSCAPI_V2_PREFIX,OSCAPI_PUT_TWEET];
		NSMutableDictionary *paramDic = @{
								   @"content"   : [Utils convertRichTextToRawYYTextView:_edittingArea],
                                   @"aboutId"   : @(self.aboutId),
                                   @"aboutType" : @(self.aboutType),
                                   @"aboutFromTweetId" : @(_fromID)
								   }.mutableCopy;
        if (_imageToken.length) {
            [paramDic setObject:_imageToken forKey:@"images"];
        }
...
- (void)sendComment{
    if ([_delegate respondsToSelector:@selector(sendCommentOfForwardWithTextView:)]) {
        [_delegate sendCommentOfForwardWithTextView:_edittingArea];
    }
}
...
- (void)pubTweet {
    ...
    if ([Utils convertRichTextToRawYYTextView:_edittingArea].length > maxStrLength) {
        MBProgressHUD* hud = [MBProgressHUD showHUDAddedTo:self.view animated:YES];
        hud.mode = MBProgressHUDModeText;
        hud.label.text = @"最多输入160个字";
    ...
    if (!_isTeamTweet) {
        if (_images.count > 0) {    //发布图片动弹
            ...
            if (_commentBtn && _commentBtn.selected) {
                [self sendComment];
            }
        }
...
- (void)pubTeamTweet {
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        [manager             POST:[NSString stringWithFormat:@"%@%@", OSCAPI_PREFIX, API]
                       parameters:@{
                                    @"uid": @([Config getOwnID]),
                                    @"msg": [Utils convertRichTextToRawYYTextView:_edittingArea],
                                    @"teamid": @(_teamID)
                                    }
        ...
        constructingBodyWithBlock:^(id<AFMultipartFormData> formData) {
                          ...
                          success:^(AFHTTPRequestOperation *operation, ONOXMLDocument *responseDocument) {
                              ...
                              if (errorCode == 1) {
                                  ...
                                  [Config saveTweetText:_edittingArea.text forUser:[Config getOwnID]];
                                  [JDStatusBarNotification showWithStatus:errorMessage dismissAfter:2.0f];
                              }
...
{
//    NSLog(@"（%ld %ld）（%ld %ld） %ld %ld ",textView.text.length,
//                                        _edittingArea.text.length,
//                                        textView.attributedText.length,
//                                        _edittingArea.attributedText.length,
//                                        range.length,text.length);
//    NSUInteger totalLength = textView.text.length - 1;
//    _textLengthLabel.text = [NSString stringWithFormat:@"%ld",totalLength];
...
- (void)textViewDidChange:(YYTextView *)textView{
    NSUInteger totalLength = [Utils convertRichTextToRawYYTextView:textView].length;
    if (totalLength > 0) {
        if (totalLength > maxStrLength) {
            _textLengthLabel.textColor = [UIColor redColor];
            NSUInteger d_value = totalLength - 160;
            _textLengthLabel.text = [NSString stringWithFormat:@"-%ld",(unsigned long)d_value];
        }else{
            _textLengthLabel.textColor = [UIColor grayColor];
            _textLengthLabel.text = [NSString stringWithFormat:@"%ld",(unsigned long)totalLength];
        }
    }
    
    _textLengthLabel.hidden = NO;
    if (_edittingArea.text.length == 0) { _textLengthLabel.hidden = YES; }
...
// 父视图UICollectionView 的长按事件。编辑框不可接受。以此保证，可以正常的长按选中文本。
