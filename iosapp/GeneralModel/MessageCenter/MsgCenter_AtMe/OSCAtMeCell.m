...
- (void)setAtMeItem:(AtMeItem *)atMeItem{
    ...
    [_userPortraitImageView loadPortrait:[NSURL URLWithString:atMeItem.author.portrait] userName:atMeItem.author.name];
    
    _nameLabel.text = atMeItem.author.name;
    
    if (atMeItem.author.identity.officialMember){
    ...
    NSMutableAttributedString *att = [[NSMutableAttributedString alloc] initWithString:[NSString stringWithFormat:@"%@", [[NSDate dateFromString:atMeItem.pubDate] timeAgoSince]]];
