...
- (void)setMessageItem:(MessageItem *)messageItem{
    ...
    [_userPortraitImageView loadPortrait:[NSURL URLWithString:messageItem.sender.portrait] userName:messageItem.sender.name];
    _userNameLabel.text = messageItem.sender.name;
    
    if (messageItem.sender.identity.officialMember) {
        _idendityLabel.hidden = NO;
    }else{
        _idendityLabel.hidden = YES;
    }
    
    _timeLabel.text = [[NSMutableAttributedString alloc] initWithString:[NSString stringWithFormat:@"%@", [[NSDate dateFromString:messageItem.pubDate] timeAgoSince]]].string;
