...
- (instancetype)init{
    self = [super init];
    if (self) {
        _strUrl = [NSString stringWithFormat:@"%@%@", OSCAPI_V2_PREFIX, OSCAPI_MESSAGES_COMMENTS_LIST];
        [self getCache];
    }
    return self;
}
...
- (void)viewDidLoad {
    ...
    if (self.dataSource && self.dataSource.count > 0) {
        ...
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1.0 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            ...
            NSTimeInterval saveTimeInterval = [(NSDate* )[[NSUserDefaults standardUserDefaults] objectForKey:userDefault_review_key_updateTime] timeIntervalSince1970];
            ...
            if (fileCreatTimeInterval > saveTimeInterval) {
                [NSObject settingTagHasBeenRead:MsgCountTypeReview];
            }
        });
    }else{
