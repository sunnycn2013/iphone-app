...
{
    NSString *ownAccount = [Config getOwnAccount];
	_userNameTextField.text = ownAccount;
...
- (IBAction)highlightedState:(UIButton *)sender {
    if (sender == _loginButton) {
        _loginButton.backgroundColor = ButtonPressedBackgroundColor;;
        
    } else if (sender == _registerButton){
        _registerButton.backgroundColor = ButtonRegisterPressedBackgroundColor;
    }
}

- (IBAction)outSideState:(UIButton *)sender {
    if (sender == _loginButton) {
        _loginButton.backgroundColor = ButtonNormalBackgroundColor;
        
    } else if (sender == _registerButton){
        _registerButton.backgroundColor = ButtonRegisterNormalBackgroundColor;
...
- (void)buttonStateEnable
{
    if (_userNameTextField.text.length > 0 && _passwordTextField.text.length > 5) {
        _loginButton.backgroundColor = ButtonNormalBackgroundColor;
        [_loginButton setTitleColor:ButtonNormalTextColor forState:UIControlStateNormal];
        _loginButton.enabled = YES;
    } else {
        _loginButton.backgroundColor = ButtonLoginDissableBackgroundColor;
        [_loginButton setTitleColor:ButtonLoginDissableTextColor forState:UIControlStateNormal];
...
- (void)saveUserInfo:(OSCUserItem *)user
{
    ...
    [Utils getNomalAttentionContacterAndSaveToAttentionContacter];
...
- (void)keyboardAction:(NSNotification *)notification
{
    ...
    maxY = self.moveBottomView.frame.origin.y - ((viewHeight - keyBoradHeight + navBarHeight - affsetHeight) * 0.5);
    ...
    if (notification.name == UIKeyboardWillShowNotification) {
        if (isShowKeyboard == NO) {
            [UIView animateWithDuration:timeInt
                             ...
                             animations:^{
                                 ...
                                 _moveBottomView.frame = [self startAnimationForEditing:_moveBottomView];
        ...
        [UIView animateWithDuration:-timeInt
                         ...
                         animations:^{
                             ...
                             _moveBottomView.frame = [self stopAnimationForEditing:_moveBottomView];
