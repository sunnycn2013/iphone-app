...
- (void)setDiscuss:(OSCDiscuss *)discuss{
    ...
    [_userPortraitImageView loadPortrait:[NSURL URLWithString:discuss.author.portrait] userName:discuss.author.name];

    _userNameLabel.text = discuss.author.name;
    
    _idendityLabel.hidden = !discuss.author.identity.officialMember;
    ...
    NSMutableAttributedString *att = [[NSMutableAttributedString alloc] initWithString:[NSString stringWithFormat:@"%@", [[NSDate dateFromString:discuss.pubDate] timeAgoSince]]];
