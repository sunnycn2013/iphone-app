...
- (void)setLayout
{
    QuartzCanvasView* drawView = [[QuartzCanvasView alloc]initWithFrame:(CGRect){{0,0},self.bounds.size} withCenterY:(self.bounds.size.height - 24)/2];
    _drawView = drawView;
    ...
    followsBtnAndFansBtn_colorLine.backgroundColor = [UIColor colorWithHex:0xffffff alpha:0.5];
...
-(void)settingLayout{
    ...
    if (_idendityLabel) {
        ...
        [_signatureLabel mas_makeConstraints:^(MASConstraintMaker *make) {
            ...
            make.top.equalTo(_idendityLabel.mas_bottom).with.offset(name_space_signature);
        }];
    }else{
        [_signatureLabel mas_makeConstraints:^(MASConstraintMaker *make) {
            make.left.equalTo(self).with.offset(20);
    ...
    [_followsBtnAndFansBtn_colorLine mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.equalTo(self).with.offset(screen_half_width);
        make.top.equalTo(_integralLabel.mas_bottom).with.offset(integral_space_centerBtn + 8);
...
{
    [_portrait loadPortrait:user.portraitURL userName:user.name];
    ...
    _signatureLabel.text = [NSString stringWithFormat:@"上次登录：%@",[user.latestOnlineTime timeAgoSince]];
...
- (void)setContentWithUserItem:(OSCUserItem *)userItem{
    ...
    _integralLabel.text = [NSString stringWithFormat:@"积分 %ld",(long)userItem.statistics.score];
    
    [_followsBtn setTitle:[NSString stringWithFormat:@"关注 %ld", (long)userItem.statistics.follow] forState:UIControlStateNormal];
    [_fansBtn setTitle:[NSString stringWithFormat:@"粉丝 %ld", (long)userItem.statistics.fans] forState:UIControlStateNormal];
    
    if (!userItem.identity.officialMember) {
        [_idendityLabel removeFromSuperview];
        _idendityLabel = nil;
        [self settingLayout];
    }else{
