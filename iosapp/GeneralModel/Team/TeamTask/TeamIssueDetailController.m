    ...
    if(indexPath.section == 0)
    {
        ...
        if (![tempDic[@"title"] isEqualToString:@"-"]) {    //根据原始设定数据判断当前cell的风格
            if ([tempDic[@"cellLevel"] intValue] == 1) {
                ...
                NSDictionary *tempDic = _originDatas[indexPath.row];
                [cell.portraitIv loadPortrait:tempDic[@"portraitUrl"] userName:tempDic[@"icon"]];
...
-(void)showIssueState
{
    ...
    UIAlertController *alertController = [UIAlertController alertControllerWithTitle:@"选择任务状态" message:showString preferredStyle:UIAlertControllerStyleAlert];
    [alertController addAction:[UIAlertAction actionWithTitle:@"取消" style:UIAlertActionStyleCancel handler:^(UIAlertAction * _Nonnull action) {
        return ;
    }]];
    
    for (NSString *stateString in [allState allValues]) {
        [alertController addAction:[UIAlertAction actionWithTitle:stateString style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
            NSString *newState = [self getStateWithChineseName:stateString];
