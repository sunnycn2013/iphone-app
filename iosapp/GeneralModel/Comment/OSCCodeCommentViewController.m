//
//  OSCCodeCommentViewController.m
//  iosapp
//
...
@interface OSCCodeCommentViewController ()<UITableViewDelegate, UITableViewDataSource, UITextFieldDelegate,OSCPopInputViewDelegate,CommentTextViewDelegate, OSCCommetCellDelegate>

@property (nonatomic, assign) NSInteger sourceId;
@property (nonatomic, strong) NSString *name;
@property (nonatomic, strong) NSString *idStr;//请求id

@property (nonatomic, strong) NSString *url;//发表评论的URL字段
...
@implementation OSCCodeCommentViewController
...
- (void)getCommentData:(BOOL)isRefresh
{
    ...
    NSString *URLStr = [NSString stringWithFormat:@"%@%@", OSCAPI_V2_PREFIX, @"gist_comments_list"];
    AFHTTPRequestOperationManager* manger = [AFHTTPRequestOperationManager OSCJsonManager];
    if (isRefresh) {
        _nextPageToken = @"";
    }
    [manger GET:URLStr
     parameters:@{@"gistId":self.idStr,@"pageToken":_nextPageToken} success:^(AFHTTPRequestOperation * _Nonnull operation, id  _Nonnull responseObject) {
...
- (void)sendCommentWithString:(NSString *)commentStr
{
    ...
    [manger POST:[NSString stringWithFormat:@"%@%@", OSCAPI_V2_PREFIX, @"pub_gist_comment"]
      parameters:@{
                   @"gistId"   : self.idStr,
                   @"name"     : _name,
                   @"content"  : commentStr,
