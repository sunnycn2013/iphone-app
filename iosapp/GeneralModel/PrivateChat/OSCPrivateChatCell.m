...
- (void)setPrivateChatItem:(OSCPrivateChat *)privateChatItem{
    ...
    [_photoView sd_setImageWithURL:[NSURL URLWithString:privateChatItem.resource] placeholderImage:[UIImage imageWithColor:[[UIColor grayColor] colorWithAlphaComponent:0.6]] options:SDWebImageContinueInBackground | SDWebImageHandleCookies completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, NSURL *imageURL) {
        NSArray* resultWH = [weakSelf fullScreenSize:image];
        objc_setAssociatedObject(_photoView, @"__GAResultSize", resultWH, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
...
//        _photoView.image = [self imageTip];
    ...
    _timeTipFrame = privateChatItem.timeTipFrame;
    _rowHeight = privateChatItem.rowHeight;
    self.height = _popFrame.size.height + SCREEN_PADDING_TOP + SCREEN_PADDING_BOTTOM;
}

#pragma mark --- 图片大小 & 遮罩处理
- (NSArray* )fullScreenSize:(UIImage* )image{
    if (!image) {return @[];}
    CGFloat targetW = [UIScreen mainScreen].bounds.size.width;
    CGFloat targetH = targetW / (image.size.width / image.size.height);
...
//        currentPhotoItem.thumbView = fromView;
        NSArray* resultWH = objc_getAssociatedObject(_photoView, @"__GAResultSize");
        if (resultWH) {
            CGFloat w = [resultWH.firstObject floatValue];
            CGFloat h = [resultWH.lastObject  floatValue];
            currentPhotoItem.largeImageSize = (CGSize){w,h};
        }
