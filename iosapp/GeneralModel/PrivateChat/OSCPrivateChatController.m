...
- (void)getDataUpgradeRequest:(BOOL)isUpgradeReq{
    NSString *strUrl = [NSString stringWithFormat:@"%@%@?uid=%ld&authorId=%ld",OSCAPI_V2_PREFIX,OSCAPI_MESSAGE_CHAT_LIST,(long)[Config getOwnID],_authorId];
      ...
      parameters:paraMutableDic.copy
         success:^(AFHTTPRequestOperation * _Nonnull operation, id  _Nonnull responseObject) {
             if ([responseObject[@"code"]integerValue] == 1) {
                 ...
                 NSArray* models = [NSArray osc_modelArrayWithClass:[OSCPrivateChat class] json:items];
                 for (OSCPrivateChat *chatModel in models) {
                     chatModel.content = [Utils convertStringToNameWithString:chatModel.content];
                 }
