...
@implementation OSCCodeSnippetListModel
//绑定信息
+ (NSDictionary *)modelCustomPropertyMapper{
    return @{@"code_description":@"description",
             @"id_str":@"id"};
}

+ (NSDictionary *)modelContainerPropertyGenericClass{
    return @{@"owner":[CodeOwner class]};
}
...
- (void)parmerInfoAttribute{
    if (!_infoAttribute) {
        ...
        UIImage *langImag = [UIImage imageNamed:@"ic_comment"];
//        
...
//        [_infoAttribute appendAttributedString:[[NSAttributedString alloc] initWithString:[NSString stringWithFormat:@" %ld   ", _comments_count]]];
...
- (void)calculateLayoutWithCurTweetCellWidth:(CGFloat)curWidth{
    ...
    if (_code_description.length > 0) {
        
        float descriptionHeight = [self getSizeWithString:_code_description withFont:[UIFont systemFontOfSize:14] withSize:layoutSize].height;
        ...
        _descriptionFrame = CGRectMake(kRightPaddingLeft, descriptionY, 0, 0);
    }
  
    float infoY = CGRectGetMaxY(_descriptionFrame) + kDescription_padding_Info;
