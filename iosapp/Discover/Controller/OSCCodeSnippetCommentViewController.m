//
//  OSCCodeSnippetCommentViewController.m
//  iosapp
//
//  Created by wupei on 2017/5/11.
//  Copyright © 2017年 oschina. All rights reserved.
//

#import "OSCCodeSnippetCommentViewController.h"
...
@interface OSCCodeSnippetCommentViewController ()<UITableViewDelegate, UITableViewDataSource, UITextFieldDelegate,OSCPopInputViewDelegate,CommentTextViewDelegate, OSCCommetCellDelegate>
...
@implementation OSCCodeSnippetCommentViewController
...
- (void)getCommentData:(BOOL)isRefresh
{
    ...
    NSString *URLStr = [NSString stringWithFormat:@"%@%@", OSCAPI_V2_PREFIX, @"git_comments_list"];
    AFHTTPRequestOperationManager* manger = [AFHTTPRequestOperationManager OSCJsonManager];
    if (isRefresh) {
        _nextPageToken = @"";
    }
    [manger GET:URLStr
     parameters:@{@"projectId":@(_sourceId),@"pageToken":_nextPageToken} success:^(AFHTTPRequestOperation * _Nonnull operation, id  _Nonnull responseObject) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (_comments.count) {
        ...
        OSCCommetCell *commentCell = [OSCCommetCell commetCellWithTableView:self.tableView
                                                        ...
                                                        commentUxiliaryNode:CommentUxiliaryNode_none
                                                            isNeedReference:NO];
...
- (void)sendCommentWithString:(NSString *)commentStr
{
    ...
    [manger POST:[NSString stringWithFormat:@"%@%@", OSCAPI_V2_PREFIX, @"pub_git_comment"]
      parameters:@{
                   @"projectId"   : @(self.sourceId),
                   @"name"       : _name,
                   @"pathWithNamespace" : _nameSpace,
                   @"content"    : commentStr,
                   }
...
- (OSCPopInputView *)inputView{
    if(!_inputView){
        ...
        _inputView.popInputViewType = 0;
        _inputView.draftKeyID = [NSString stringWithFormat:@"Git_%ld",(long)_sourceId];
    }
    return _inputView;
}
