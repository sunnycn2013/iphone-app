...
- (void)setRandomMessageItem:(OSCRandomMessageItem *)randomMessageItem{
    ...
    [self.msgImageView loadPortrait:[NSURL URLWithString:randomMessageItem.img] userName:randomMessageItem.name];
    self.descLabel.text = randomMessageItem.name;
    self.timeLabel.text = [NSString stringWithFormat:@"发布于 %@",[[NSDate dateFromString:randomMessageItem.pubDate] timeAgoSince]];
}
