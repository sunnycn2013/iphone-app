...
//                  HUD.customView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"HUD-error"]];
    ...
    __block UITextField *reTextField = [UITextField new];
    __block NSString *weakWebUrl = weakSelf.webURL;
    self.operationBar.report = ^ {
        UIAlertController *alertController = [UIAlertController alertControllerWithTitle:@"举报" message:[NSString stringWithFormat:@"链接地址：%@", weakSelf.webURL] preferredStyle:UIAlertControllerStyleAlert];
        [alertController addTextFieldWithConfigurationHandler:^(UITextField * _Nonnull textField) {
            textField.placeholder = @"举报原因";
            reTextField = textField;
        }];
        ...
        [alertController addAction:[UIAlertAction actionWithTitle:@"确定" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
            ...
            [manager POST:@"http://www.oschina.net/action/communityManage/report"
               parameters:@{
                            @"memo":        reTextField.text.length == 0? @"其他原因": reTextField.text,
                            @"obj_id":      @(weakSelf.objectID),
                            @"obj_type":    @"2",
                            @"reason":      @"4",
                            @"url":         weakWebUrl,
                            }
