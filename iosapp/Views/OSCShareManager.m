...
- (void)showShareBoardWithGitModel:(OSCGitDetailModel *)model{
    ...
    OSCShareBoard *curShareBoard = [OSCShareBoard shareBoardWithGitModel:model path:nil];
    _curShareBoard = curShareBoard;
    curShareBoard.frame = [UIScreen mainScreen].bounds;
    curShareBoard = curShareBoard;
    curShareBoard.delegate = self;
...
- (void)showShareBoardWithCodeModel:(OSCCodeSnippetListModel *)model{
    ...
    OSCShareBoard *curShareBoard = [OSCShareBoard shareBoardWithCodeModel:model path:nil];
...
- (void)showShareBoardWithGitCodeModel:(OSCGitDetailModel *)model path:(NSString* )path{
    ...
    OSCShareBoard *curShareBoard = [OSCShareBoard shareBoardWithGitModel:model path:path];
    _curShareBoard = curShareBoard;
    curShareBoard.frame = [UIScreen mainScreen].bounds;
    curShareBoard = curShareBoard;
    curShareBoard.delegate = self;
...
- (void)showShareBoardWithImage:(UIImage *)image{
    ...
    OSCShareBoard *curShareBoard = [OSCShareBoard shareBoardWithImage:image];
    _curShareBoard = curShareBoard;
    curShareBoard.frame = [UIScreen mainScreen].bounds;
    curShareBoard = curShareBoard;
    curShareBoard.delegate = self;
				   ...
				   model:(id)model
{
    ...
    switch (infomationType) {
        case InformationTypeTweet://动弹（评论）类型
        {
            ...
            NSInteger length = trimmedHTML.length < 30 ? trimmedHTML.length : 30;
...
+ (instancetype)shareBoardWithCodeModel:(OSCCodeSnippetListModel *)model path:(NSString* )path{
    ...
    curShareBoard.isGitShare = YES;
    curShareBoard.isImage = NO;
    [curShareBoard settingCodeModel:model path:path];
    return curShareBoard;
}

- (void)settingCodeModel:(OSCCodeSnippetListModel *)model path:(NSString* )path{
    self.logoImage = [UIImage imageNamed:@"ios_120"];
    OSCCodeSnippetListModel *questionItem = (OSCCodeSnippetListModel *)model;
//    NSString *trimmedHTML = [questionItem.content deleteHTMLTag];
//    NSInteger length = trimmedHTML.length < 100 ? trimmedHTML.length : 100;
    
    self.title = [NSString stringWithFormat:@"%@/%@",questionItem.owner.name,questionItem.name];
//    self.digest = [trimmedHTML substringToIndex:length];
    
    self.href = path ?: [NSString stringWithFormat:@"%@",questionItem.url];
    
//    self.aboutId = questionItem.id;
//    self.descString = [self stringReplayOcc:trimmedHTML];
    
    self.descString = questionItem.code_description;
}
