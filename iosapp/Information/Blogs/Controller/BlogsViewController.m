...
- (instancetype)initWithUserID:(int64_t)userID
{
    if (self = [super init]) {
        self.generateURL = ^NSString * (NSUInteger page) {
            return [NSString stringWithFormat:@"%@%@?authoruid=%lld&pageIndex=%lu&uid=%ld", OSCAPI_PREFIX, OSCAPI_USERBLOGS_LIST, userID, (unsigned long)page, (long)[Config getOwnID]];
        };
        self.objClass = [OSCBlog class];
    }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    NewBlogDetailController* newsBlogDetailVc = [[NewBlogDetailController alloc] initWithDetailId:blog.blogID];
