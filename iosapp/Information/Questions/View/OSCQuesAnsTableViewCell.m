...
- (void)setListItem:(OSCListItem *)listItem
{
    ...
    [portraitIMView loadPortrait:[NSURL URLWithString:_listItem.author.portrait] userName:_listItem.author.name];
    titleLabel.text = _listItem.title;
    descLabel.text = (_listItem.body.length > 0 ? _listItem.body : @" ");
    
    nameLabel.text = [NSString stringWithFormat:@"@%@", _listItem.author.name];
    NSMutableAttributedString *att = [[NSMutableAttributedString alloc] initWithString:[NSString stringWithFormat:@"%@",[[NSDate dateFromString:listItem.pubDate] timeAgoSince]]];
    ...
    [timeLabel setAttributedText:att];
