...
#define iconImageView_Height 14
...
- (void)layoutSubviews{
    ...
    _authorAndTimeLb_width = CGRectGetMinX(_commentCountImageView.frame) - padding_left;
}


- (void)setQuestion:(OSCListItem *)question{
    _titleLb.text = question.title;
    _titleLB_Height = [self getHeightWithString:question.title WithFont:[UIFont systemFontOfSize:22]];
    _tagLb.attributedText = [self setTagAttributedString:question.tags];
	_authorAndTimeLb.text = [NSString stringWithFormat:@"%@ %@", question.author.name, [[NSDate dateFromString:question.pubDate] timeAgoSince]];
    _commentCountLb.text = [NSString stringWithFormat:@"%ld", (long)question.statistics.comment];
...
- (CGFloat)getHeightWithString:(NSString *)string WithFont:(UIFont *)font{
    return [string sizeForFont:font size:CGSizeMake(_fullWidth, MAXFLOAT) mode:NSLineBreakByWordWrapping].height + 5;
}
...
- (void)touchesBegan:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event{
    ...
    CGPoint p = [t locationInView:_titleLb];
    if (CGRectContainsPoint(_titleLb.bounds, p)) {
        [GAMenuView MenuViewWithTitle:@"复制" block:^{
            UIPasteboard* pasteboard = [UIPasteboard generalPasteboard];
            [pasteboard setString:_titleLb.text];
        } inView:_titleLb];
    }
}
