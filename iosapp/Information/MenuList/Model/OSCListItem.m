...
+ (NSDictionary *)modelContainerPropertyGenericClass{
    return @{
             ...
             @"abouts" : [OSCAbout class],
             };
}
- (BOOL)modelCustomTransformFromDictionary:(NSDictionary *)dic {
    NSArray* tags = dic[@"tags"];
    [self todayIconAttributedString];
...
#pragma TODO : boundingRectWithSize: 计算不准
            ...
            /** nameLb */
            CGSize nameSize = (CGSize)[[NSString stringWithFormat:@"@%@", _author.name] sizeWithAttributes:@{NSFontAttributeName : [UIFont systemFontOfSize:blogsCell_infoLB_Font_Size]}];
...
//            
//            CGSize timeSize = (CGSize)[[date timeAgoSince] sizeWithAttributes:@{NSFontAttributeName : [UIFont systemFontOfSize:blogsCell_infoLB_Font_Size]} ];
            
            blogInfo.timeLbFrame = (CGRect){{cell_padding_left + nameSize.width + 5, cell_padding_top + titleSize.height + blogsCell_titleLB_SPACE_descLB + descSize.height + blogsCell_descLB_SPACE_infoBar}, {blogInfo.viewCountImgFrame.origin.x - (cell_padding_left + nameSize.width + 5) - 10, blogsCell_infoBar_Height}};
        ...
        case InformationTypeForum:{
            if ([_menuItem.token isEqualToString:@"d6112fa662bc4bf21084670a857fbd20"]) {//推荐列表 问答
                ...
                /** nameLb */
                CGSize nameSize = (CGSize)[[NSString stringWithFormat:@"@%@", _author.name] sizeWithAttributes:@{NSFontAttributeName : [UIFont systemFontOfSize:questionsCell_infoLB_Font_Size]}];
...
//                NSDate* date = [formatter dateFromString:_pubDate];
//                
//                CGSize timeSize = (CGSize)[[date timeAgoSince] sizeWithAttributes:@{NSFontAttributeName : [UIFont systemFontOfSize:questionsCell_infoLB_Font_Size]} ];
                
                questionInfo.timeLbFrame = (CGRect){{questionsCell_descLB_Padding_Left + nameSize.width + 5, cell_padding_top + titleSize.height + questionsCell_titleLB_SPACE_descLB + descSize.height + questionsCell_descLB_SPACE_infoBar}, {questionInfo.viewCountImgFrame.origin.x - questionsCell_descLB_Padding_Left - nameSize.width - 5 - 10, questionsCell_infoBar_Height}};
...
- (CGFloat)infomationLayoutHeight:(CGFloat)rowHeight
{
    ...
    /** viewCountLb */
    CGSize viewsSize = (CGSize)[[NSString stringWithFormat:@"%ld", (long)_statistics.view] sizeWithAttributes:@{NSFontAttributeName : [UIFont systemFontOfSize:informationCell_infoBar_Font_Size]}];
    infoLayoutInfo.viewCountLbFrame = (CGRect){{kScreen_bound_width - cell_padding_right - commentSize.width - informationCell_icon_width - informationCell_count_icon_count_space - viewsSize.width - informationCell_icon_count_icon_space, cell_padding_top + titleSize.height + informationCell_titleLB_SPACE_descLB + descSize.height + informationCell_descLB_SPACE_infoBar}, {viewsSize.width, informationCell_infoBar_Height}};
    /** viewCountImageView */
    infoLayoutInfo.viewCountImgFrame = (CGRect){{kScreen_bound_width - cell_padding_right - commentSize.width - informationCell_icon_width - informationCell_count_icon_count_space - viewsSize.width - questionsCell_icon_count_icon_space - informationCell_icon_width - informationCell_count_icon_count_space, cell_padding_top + titleSize.height + informationCell_titleLB_SPACE_descLB + descSize.height + informationCell_descLB_SPACE_infoBar + informationCell_count_icon_count_space}, {informationCell_icon_width, informationCell_icon_height}};
...
//    CGSize timeSize = (CGSize)[[date timeAgoSinceNow] sizeWithAttributes:@{NSFontAttributeName : [UIFont systemFontOfSize:informationCell_infoBar_Font_Size]} ];
    ...
    infoLayoutInfo.timeLbFrame = (CGRect){{cell_padding_left, cell_padding_top + titleSize.height + informationCell_titleLB_SPACE_descLB + descSize.height + informationCell_descLB_SPACE_infoBar}, {infoLayoutInfo.commentImgFrame.origin.x - (cell_padding_left) - 10, informationCell_infoBar_Height}};
...
- (NSDictionary* )getAuthorJSON{
    _name       = !_name || [_name isEqual:[NSNull null]] ? @" " : _name;
    _portrait   = !_portrait || [_portrait isEqual:[NSNull null]] ? @" " : _portrait;
    return @{
             @"id"          : @(_id),
             @"name"        : _name,
             @"portrait"    : _portrait,
...
+ (instancetype)authorWithJSON:(NSDictionary* )dic{
    ...
    author.id       = [dic[@"id"] integerValue];
    author.name     = dic[@"name"];
    author.portrait = dic[@"portrait"];
    author.relation = [dic[@"relation"] integerValue];
    return author;
}

@end

@implementation OSCRecommendSoftware
