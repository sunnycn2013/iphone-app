...
- (void)configurationBannerView{
    switch (_menuItem.banner.catalog) {
        case OSCInformationListBannerTypeSimple:
        case OSCInformationListBannerTypeSimple_Blogs:{
            self.bannerScrollView.banners = [self _convertModels:[self.bannerDataSources copy]];
            break;
        }
...
- (NSArray<OSCBannerModel* >* )_convertModels:(NSArray<OSCBanner* >* )banners{
    if (!banners || [banners isEqual:[NSNull null]] || banners.count == 0) return nil;
    
    NSMutableArray<OSCBannerModel* >* mArr = [NSMutableArray arrayWithCapacity:banners.count];
    for (OSCBanner* banner in banners) {
        OSCBannerModel* model = [OSCBannerModel bannerModelWithTitle:banner.name netImagePath:banner.img localImageData:nil];
        ...
        success:^(AFHTTPRequestOperation * _Nonnull operation, id  _Nonnull responseObject) {
            if ([responseObject[@"code"] integerValue] == 1) {
                ...
                if (responseArr && responseArr.count > 0) {
                    ...
                    [NSObject handleResponseObject:resultDic resource:bannerResourceName cacheType:SandboxCacheType_banner];
                }
                /***********************/
                                                        ...
                                                        indexPaht:(NSIndexPath* )indexPath
{
    switch (listItem.type) {
        ...
        case InformationTypeForum:{
            if ([_menuItem.token isEqualToString:@"d6112fa662bc4bf21084670a857fbd20"]) {//推荐
                 OSCInformationTableViewCell *curCell = [tableView dequeueReusableCellWithIdentifier:InformationTableViewCell_IdentifierString forIndexPath:indexPath];
                curCell.showCommentCount = YES;
        ...
        default:{
            ...
            OSCInformationTableViewCell *curCell = [tableView dequeueReusableCellWithIdentifier:InformationTableViewCell_IdentifierString forIndexPath:indexPath];
            if (listItem.type == InformationTypeLinkNews || _menuItem.type == 7) { //链接新闻
...
- (BannerScrollView *)bannerScrollView{
    if (!_bannerScrollView) {
        BannerScrollViewConfiguration* config = [BannerScrollViewConfiguration quickConfigurationTitleFontSize:15 titleColor:[UIColor whiteColor] pageSelectedColor:[UIColor greenColor] style:BannerScrollViewStyleAuto];
        _bannerScrollView = [BannerScrollView bannerScrollViewWithFrame:CGRectMake(0, 0, kScreen_Width, BannerView_Simple_Height) configuration:config models:[self _convertModels:[self.bannerDataSources copy]]];
