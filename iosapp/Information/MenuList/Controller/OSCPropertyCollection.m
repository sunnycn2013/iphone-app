...
-(instancetype)initWithFrame:(CGRect)frame WithSelectIndex:(NSInteger)index{
    ...
    if(self){
        ...
        NSArray *fixArray = [Utils fixedLocalMenuNames];
        NSArray *selectArray = [Utils allSelectedMenuNames];
        _selectTitle = [[[fixArray mutableCopy] arrayByAddingObjectsFromArray:selectArray] mutableCopy];
...
-(void)moveCellWithGR:(UILongPressGestureRecognizer *)longGR{
    if (longGR.state == UIGestureRecognizerStateBegan) {
        ...
        BOOL isNull = YES;
        for (int i = 0; i < 4; i ++ ) {
            CGRect rectOfCell = [[self cellForItemAtIndexPath:[NSIndexPath indexPathForRow:i inSection:0]] frame];
            isNull = isNull && CGRectIsNull(CGRectIntersection(rectOfCurrent, rectOfCell));
        }
        if (isNull) {
