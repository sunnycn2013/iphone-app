...
@interface OSCSyntheticalController () <SyntheticalTitleBarDelegate,OSCPropertyCollectionDelegate,InformationListCollectionDelegate,CheckVersionViewDelegate>
...
@property (nonatomic, strong) CheckVersionView *checkV;
...
- (void)checkVersion {
    ...
    //当前APP版本号
    NSInteger curVersionInt = [[[NSBundle mainBundle].infoDictionary objectForKey:@"CFBundleVersion"] integerValue];
    
    AFHTTPRequestOperationManager *manager = [AFHTTPRequestOperationManager OSCJsonManager];
    NSString *urlStr = [NSString stringWithFormat:@"%@",@"https://www.oschina.net/action/apiv2/product_version?appId=1&catalog=2"];
    ...
    [manager GET:urlStr parameters:nil success:^(AFHTTPRequestOperation * _Nonnull operation, id  _Nonnull responseObject) {
        ...
        if ([responseObject[@"code"] integerValue] == 1) {
            ...
            NSInteger resVersionInt = [(NSString *)result[0][@"code"] integerValue];
        
            
            if (resVersionInt > curVersionInt) {//服务器的版本更大
            
                self.updateUrl = result[0][@"downloadUrl"];
            
            
            NSString *newStr = [result[0][@"message"] stringByReplacingOccurrencesOfString:@" <br/>" withString:@""];
...
- (void)showUpdateView {
    ...
    CheckVersionView *checkV = [[[UINib nibWithNibName:@"CheckVersionView" bundle:nil] instantiateWithOwner:nil options:nil] lastObject];
    checkV.contentView.text = weakSelf.message;
    
    checkV.contentView.editable = NO;
    
    [checkV.updateBtn setBackgroundImage:[self imageWithColor:[UIColor colorWithHex:0x188E50]] forState:UIControlStateHighlighted];
    
    checkV.frame = CGRectMake(0, 0, [UIScreen mainScreen].bounds.size.width - 75, 350);
    
    checkV.center = bgView.center;
    checkV.delegate = weakSelf;
    ...
    weakSelf.checkV = checkV;
    [window addSubview:checkV];
}
- (void)updateBtnClick {
    //跳转到appstore
...
- (void)closeBtnClick {
    ...
    [UIView animateWithDuration:0.5 animations:^{
        ...
        [self.checkV  removeFromSuperview];
    }];
}
