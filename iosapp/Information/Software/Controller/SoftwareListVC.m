...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    /* 新版软件详情 */
    SoftWareViewController *detailSoftwareVC = nil;
    if (software.softId == 0 || software.softId == NSNotFound) {
        if ([[software.url absoluteString] containsString:@"/p/"]) {
            NSString* identifier = [[software.url absoluteString] lastPathComponent];
            identifier = [identifier stringByReplacingOccurrencesOfString:@"+" withString:@" "];
            detailSoftwareVC = [[SoftWareViewController alloc] initWithSoftWareIdentity:identifier];
        }
    }else{
        detailSoftwareVC = [[SoftWareViewController alloc]initWithSoftWareID:software.softId];
    }
