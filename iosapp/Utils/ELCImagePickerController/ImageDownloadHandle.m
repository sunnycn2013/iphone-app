...
/** YYWebImage */
+ (UIImage *) retrieveMemoryAndDisk_YYWebImage_Cache:(NSString* )imageKey{
    UIImage* image = [[YYWebImageManager sharedManager].cache getImageForKey:imageKey];
    if (!image) {
        return nil;
    }else{
        return image;
...
/** SDWebImage */
+ (UIImage *) retrieveMemoryAndDisk_SDWebImage_Cache:(NSString* )imageKey{
    UIImage* image = [[SDImageCache sharedImageCache] imageFromMemoryCacheForKey:imageKey];
    if (!image) {
        image = [[SDImageCache sharedImageCache] imageFromDiskCacheForKey:imageKey];
        if (image) {
