...
+ (UIViewController *)pushControllerWithDiscussOriginType:(OSCDiscussOrigin *)discussOrigin{
    switch (discussOrigin.type) {
        ...
        case OSCDiscusOriginTypeForum:{
            QuesAnsDetailViewController *detailVC = [[QuesAnsDetailViewController alloc] initWithDetailID:discussOrigin.id];
            detailVC.hidesBottomBarWhenPushed = YES;
            return detailVC;
            break;
        }
        case OSCDiscusOriginTypeBlog:{
            NewBlogDetailController* blogDetailVC = [[NewBlogDetailController alloc] initWithDetailId:discussOrigin.id];
            blogDetailVC.hidesBottomBarWhenPushed = YES;
            return blogDetailVC;
            break;
        }
        case OSCDiscusOriginTypeTranslation:{
            TranslationViewController *translationVc = [[TranslationViewController alloc] initWithTranslationID:discussOrigin.id];
...
+ (UIViewController *)pushControllerWithRandomNewsType:(OSCRandomMessageItem *)randomMessageItem{
    switch (randomMessageItem.type) {
        ...
        case OSCRandomMessageTypeDiscuss:{
            QuesAnsDetailViewController *detailVC = [[QuesAnsDetailViewController alloc] initWithDetailID:randomMessageItem.id];
            detailVC.hidesBottomBarWhenPushed = YES;
            return detailVC;
            break;
        }
        ...
        case OSCRandomMessageTypeBlog:{
            NewBlogDetailController* blogDetailVC = [[NewBlogDetailController alloc] initWithDetailId:randomMessageItem.id];
            blogDetailVC.hidesBottomBarWhenPushed = YES;
            return blogDetailVC;
            break;
        }
        ...
        case OSCRandomMessageTypeTranslation:{
            TranslationViewController *translationVc = [[TranslationViewController init] initWithTranslationID:randomMessageItem.id];
            translationVc.hidesBottomBarWhenPushed = YES;
    ...
    if ([searchModel isKindOfClass:[OSCSearchItem class]]) {
        ...
        switch (model.type) {
            case InformationTypeBlog:{
                NewBlogDetailController* blogDetailVC = [[NewBlogDetailController alloc] initWithDetailId:model.id];
                blogDetailVC.hidesBottomBarWhenPushed = YES;
                return blogDetailVC;
                break;
            }
            ...
            case InformationTypeForum:{
                QuesAnsDetailViewController *detailVC = [[QuesAnsDetailViewController alloc] initWithDetailID:model.id];
                detailVC.hidesBottomBarWhenPushed = YES;
                return detailVC;
                break;
            }
...
+ (UIViewController *)pushControllerWithListItem:(OSCListItem *)listItem{
    switch (listItem.type) {
        ...
        case InformationTypeForum:{
            QuesAnsDetailViewController *detailVC = [[QuesAnsDetailViewController alloc] initWithDetailID:listItem.id];
            detailVC.hidesBottomBarWhenPushed = YES;
            return detailVC;
            break;
        }
        ...
        case InformationTypeBlog:{
            NewBlogDetailController* blogDetailVC = [[NewBlogDetailController alloc] initWithDetailId:listItem.id];
            blogDetailVC.hidesBottomBarWhenPushed = YES;
            return blogDetailVC;
            break;
        }
        ...
        case InformationTypeTranslation:{
            TranslationViewController *translationVc = [[TranslationViewController alloc] initWithTranslationID:listItem.id];
            translationVc.hidesBottomBarWhenPushed = YES;
...
+ (void)pushControllerGeneralWithInformationType:(InformationType)type
                                 detailContentID:(NSInteger)id
                                            href:(NSString *)hrefString
{
    
    UIViewController* curVC = [self topViewControllerForViewController:[[UIApplication sharedApplication].keyWindow rootViewController]];
    UIViewController* detailVC = [self pushControllerGeneralWithType:type detailContentID:id];
    if(detailVC){
        [curVC.navigationController pushViewController:detailVC animated:YES];
    }else{
        [curVC.navigationController handleURL:[NSURL URLWithString:hrefString] name:nil];
    }
}
